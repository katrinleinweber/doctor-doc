...
><!--
  ...
  <xsl:param name="stylesheet">
    <xsl:choose>
      ...
      <xsl:otherwise>jsp/import/daia.css</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        ...
        <!-- footer -->
        <div id="footer">
          See <a href="http://purl.org/NET/DAIA" target="_blank">http://purl.org/NET/DAIA</a>
  ...
  <xsl:template match="d:daia">
    <p> 
      ...
      <xsl:if test="@timestamp or @version">
        ...
        <xsl:if test="@timestamp">
          ...
          <xsl:if test="@version"> and </xsl:if>
        </xsl:if>
        <xsl:if test="@version">
          is encoded in
    ...
    <xsl:if test="$docs">
      <xsl:choose>
        <xsl:when test="count($docs) = 1">
          <h2><xsl:value-of select="count($docs)"/> available holding</h2>
          <!-- xsl:apply-templates select="d:document"/-->
        </xsl:when>
        <xsl:otherwise>
          <h2><xsl:value-of select="count($docs)"/> available holdings</h2>
          <!-- 
    ...
    <xsl:if test="$items">
      <h2>Details</h2>
        <table border="1">
          <tr>
			<th><div class="document-icon">Title</div></th>
			<th><div class="item-icon">Library</div></th>
            <th><div class="item-icon">Order information</div></th>
            <th><div class="item-icon">Holding information</div></th>
            <th><div class="location-icon">All holdings of location</div></th>
            <th><div class="loan-icon">Loan</div></th>
            <th><div class="presentation-icon">Presentation</div></th>
            <th><div class="openaccess-icon">OpenAccess</div></th>
  ...
  <xsl:template match="d:document">
    <p>
      <xsl:if test="d:item">
        ...
        <xsl:text> information for request at </xsl:text>
  ...
  <xsl:template match="d:item">
    ...
    <tr>
      <xsl:if test="$department_position = 1">
        ...
        <td>
        	<xsl:value-of select="d:department"/>
        </td>
        <td>
        ...
        <div>
          ...
          <xsl:variable name="stock_href" select="@href"/>
          <a href="{$stock_href}" target="_blank"><xsl:value-of select="@href"/></a>
        <!-- 
          <xsl:call-template name="content-with-optional-href">
            <xsl:with-param name="content" select="d:item" />
          </xsl:call-template>
        -->
        </div>
      </td>
        <xsl:if test="$item_position = 1">
          <td rowspan="{count(../d:item)}">
            <xsl:variable name="holding_href" select="parent::d:document/@href"/>
        	<a href="{$holding_href}" target="_blank"><xsl:value-of select="parent::d:document/@href"/></a>
      ...
      <td>
        ...
        <xsl:variable name="library_href" select="d:department/@href"/>
        <a href="{$library_href}" target="_blank"><xsl:value-of select="d:department/@href"/></a>
  ...
  <xsl:template match="d:institution">
    <h2><xsl:call-template name="content-with-optional-href"/></h2>
  </xsl:template>
  ...
  <xsl:template name="content-with-optional-href">
    ...
    <xsl:choose>
      <xsl:when test="$content and $href">
        <a href="{$href}" target="_blank"><xsl:value-of select="$content"/></a>
      </xsl:when>
      <xsl:when test="$content">
        <span><xsl:value-of select="$content"/></span>
      </xsl:when>
      <xsl:when test="$nid and $href">
        <a href="{$href}" class="id" target="_blank"><xsl:call-template name="id"/></a>
      </xsl:when>
      <xsl:when test="$nid">
  ...
  <xsl:template name="id">
    <xsl:choose>
      <xsl:when test="substring-before(@id,':') = 'http'">
        <a href="{@id}" target="_blank"><xsl:value-of select="@id"/></a>
      </xsl:when>
      <!-- minimal URI check -->
