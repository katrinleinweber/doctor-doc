...
public class Billing extends AbstractIdEntity {
		...
		try {
			...
			if (this.getId() == null) {
				pstmt = setBillingValues(cn
						.prepareStatement("INSERT INTO `billing` (`UID` , `KID` , `rechnungsgrund` , `betrag` , `waehrung` , `rechnungsnummer` , `rechnungsdatum`, `zahlungseingang`) "
								+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"));
				pstmt.execute();
			...
			// bestehende Rechnung veraendern
			else {
				pstmt = setBillingValues(cn
						.prepareStatement("UPDATE `billing` SET `UID`=? , `KID`=? , `rechnungsgrund`=? , `betrag`=? , `waehrung`=? , `rechnungsnummer`=? , `rechnungsdatum`=?, `zahlungseingang`=? "
								+ "WHERE `ID`=" + this.getId()));
	...
	public List<Billing> getBillings(Konto k, Connection cn){
		...
		ResultSet rs = null;
		try {
			pstmt = cn.prepareStatement("SELECT * FROM `billing` WHERE KID=? ORDER BY `zahlungseingang`, `rechnungsdatum`");
	...
	public void update(Connection cn) {
		...
		try {
	          ...
	          "KID=?, rechnungsgrund=?, betrag=?, waehrung=?, rechnungsnummer=?, rechnungsdatum=?, zahlungseingang=? " +
	          "WHERE `ID` = " + this.getId()));   
	...
	private PreparedStatement setBillingValues(PreparedStatement pstmt) {
		...
		if (this.getKonto()!=null)pstmt.setLong(2, this.getKonto().getId()); else pstmt.setString(2, null);
		if (this.getRechnungsgrund()!=null)pstmt.setLong(3, this.getRechnungsgrund().getId()); else pstmt.setString(3, null);
		...
		if (this.getZahlungseingang()!=null) pstmt.setDate(8, this.getZahlungseingang()); else pstmt.setString(8, null);
		} catch (Exception e) {
