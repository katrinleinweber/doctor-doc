...
//		if (k.getKontotyp()==1)	b = new Billing(k, new Text(cn, "1 Jahr " + ReadSystemConfigurations.getApplicationName() + " Enhanced") , Double.valueOf("120"), "CHF"); // Dieses Konto gibt es nicht mehr und muss daher auch nicht verrechnet werden
		if (k.getKontotyp()==2) b = new Billing(k, new Text(cn, "1 Jahr " + ReadSystemConfigurations.getApplicationName() + " Enhanced plus Fax to Mail") , Double.valueOf("210"), "CHF");
		if (k.getKontotyp()==3) b = new Billing(k, new Text(cn, "3 Monate " + ReadSystemConfigurations.getApplicationName() + " Enhanced plus Fax to Mail") , Double.valueOf("90"), "CHF");
		...
		if (b != null) {
			...
			bf.setBillingtext("\n\nUm weiterhin optimal von " + ReadSystemConfigurations.getApplicationName() + " profitieren zu können, muss der Betrag von "
					 + b.getBetrag()+ " " + b.getWaehrung()	+ " bis spätestens am " + lastpaydate + " auf dem Konto des " 
					 + "Vereins " + ReadSystemConfigurations.getApplicationName() + " eintreffen.");// Hinweis Zahlungskreuzung, laspaydate
			...
			if (b.getId()!=null){
				mh.sendMail(to,"RECHNUNG: Erneuerung Ihres Faxservers bei " + ReadSystemConfigurations.getApplicationName() + ". Ablaufdatum: " + sdf.format(k.getExpdate()), bf.getBillingtext()); 
			} else { // Hinweis senden falls keine Rechnung erstellt werden konnte
				mh.sendErrorMail("WARNUNG: Folgende Rechnung konnte nicht erstellt werden, Kunde " + k.getBibliotheksname() + " ist nicht benachrichtigt worden! Es wurde keine Rechnung gespeichert!", 
    ...
    public BillingForm prepareBillingText(Konto k, Connection cn, String lastpaydate, BillingForm bf){
    	...
    	//Rechnungsobjekt vorbereiten, noch nicht speichern da dies erst geschieht wenn die Rechnung wirklich versendet wird.
    	Billing b = new Billing();
		b = b.getLastBilling(k, cn);
		if (b==null){
			if (k.getKontotyp()==0 || k.getKontotyp()==1)	b = new Billing(k, new Text(cn, "Dienstleistung") , Double.valueOf("0"), "CHF");
			if (k.getKontotyp()==2) b = new Billing(k, new Text(cn, "1 Jahr " + ReadSystemConfigurations.getApplicationName() + " Enhanced plus Fax to Mail") , Double.valueOf("210"), "CHF");
			if (k.getKontotyp()==3) b = new Billing(k, new Text(cn, "3 Monate " + ReadSystemConfigurations.getApplicationName() + " Enhanced plus Fax to Mail") , Double.valueOf("90"), "CHF");
		}
...
//				Bestehende Kunden auf das Ablaufdatum des Kontos hinweisen, nur wenn kein manueller Rechnungstext ergänzt werden soll
				if (k.getKontotyp()!=0){ 
					if (k.getExpdate()!=null) {
						t.append("\n\nIhr Faxserver auf doctor-doc.com läuft am " + sdf.format(k.getExpdate()) + " ab.");
						t.append("\n\nBitte	überweisen Sie den Betrag innert 30 Tagen und mit Angabe der Rechnungsnummer." + "\n");
						}
				}
			}
		}		
...
//    	 Mailtext vorbereiten
		...
		bf.setBillingtext("\n\nUm weiterhin vom Faxserver auf " + ReadSystemConfigurations.getApplicationName() + " profitieren zu können, muss der Betrag von "
					 + b.getBetrag()+ " " + b.getWaehrung()	+ " bis spätestens am " + lastpaydate + " auf dem Konto des " 
					 + "Vereins " + ReadSystemConfigurations.getApplicationName() + " eintreffen." +
					 "\n\nSollte sich ihre Zahlung mit dieser Mail gekreuzt haben, betrachten Sie dieses Mail bitte als gegenstandslos.");// Hinweis Zahlungskreuzung, laspaydate
    	bf = this.prepareBillingText(k, cn, lastpaydate, bf);
    	...
    	// Rechnung versenden
			...
			mh.sendErrorMail("ZAHLUNGSERINNERUNG 30 Tage vor Ablauf: von Kunde " + k.getBibliotheksname() + " musste nicht versendet werden.", 
					"Keine offene Rechnung gefunden, brav alles bezahlt :-)\n\nBitte Kontoablaufdatum noch richtig setzen, das gieng wohl " +
					"vergessen beim Zahlungseingang setzen...");
		}
    }
...
//   	 Mailtext vorbereiten
		...
		if (b != null) {
		bf.setManuelltext("Wir konnten leider keinen rechtzeitigen Zahlungseingang ihres ausstehenden Betrages bei uns verzeichnen. " +
				"Wir weisen Sie darauf hin, dass wir aus administrativen Gründen nicht gewährleisten können, dass Ihnen ihr Faxserver unterbrechungsfrei " +
				"zur Verfügung steht. Sobald der offene Betrag bei uns eingegangen ist, reaktivieren wir Ihnen Ihr, in der Zwischenzeit ggf. " +
				"deaktivierter, Faxserver auf doctor-doc.com wieder");
		bf.setBillingtext("Sollte sich ihre Zahlung mit dieser Mail gekreuzt haben, betrachten Sie dieses Mail bitte als gegenstandslos.");// Hinweis Zahlungskreuzung
    	bf = this.prepareBillingText(k, cn, lastpaydate, bf);
...
//    	 Rechnung versenden		
			...
			mh.sendErrorMail("LETZTE ZAHLUNGSERINNERUNG: von Kunde " + k.getBibliotheksname() + " musste nicht versendet werden.!", 
					"Keine offene Rechnung gefunden, brav alles bezahlt :-)\n\nBitte Kontoablaufdatum noch richtig setzen, das gieng wohl " +
					"vergessen beim Zahlungseingang setzen...");
		}
    }
...
//    	 Offene Rechnung heraussuchen
    	...
    	BillingForm bf = new BillingForm();
    	if (b != null) {
    	bf.setManuelltext("Der Faxserver ihres Bibliothekskonto wurde automatisch deaktiviert, da kein rechtzeitiger Zahlungseingang " +
    			"der unten stehenden Rechnung verzeichnet werden konnte. Sobald die Rechnung beglichen wird, können wir ihren Faxserver wieder freischalten (Keine Nummergarantie).");
    	bf = this.prepareBillingText(k, cn, null, bf);
    	} 
    	
//    	Eigentlich sollte man nie ohne offene Rechnung hier landen, ansonsten ist vermutlich das Expire Date des Kontos falsch gesetzt
    	else { 
    		bf.setBillingtext("Sehr geehrte Damen und Herren\n\n" +
    				"Leider mussten wir aufgrund eines internen Fehlers in " + ReadSystemConfigurations.getApplicationName() + " ihren Faxserver deaktivieren. " +
    				"Wir bemühen uns, diesen Fehler schnellstmöglich zu beheben um den Faxserver wieder frei geben zu können.\n\n" +
    				"Freundliche Grüsse:\n\n" +
    				"Ihr Team " + ReadSystemConfigurations.getApplicationName());
    	}
		...
		mh.sendErrorMail("Hinweis: Der Faxserver vom Konto " + k.getBibliotheksname() + " auf doctor-doc.com wurde deaktiviert.", 
