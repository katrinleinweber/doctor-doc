...
public class Auth {
    ...
    /**
     * Überprüft, ob ein Zugriff berechtigt erfolgt, falls der Kunde nicht eingeloggt ist {@link ch.dbs.form.UserInfo}<p>
     * 
     * @param rq HttpServletRequest
     * @return t Text
     */     
    ...
    public boolean isBestellopt(HttpServletRequest rq){
        ...
        if (rq.getSession().getAttribute("userinfo")!=null){
            ...
            if (ui.getBenutzer()!=null){
            	...
            	if (ui.getBenutzer().isUserbestellung()==true)
            		check = true;
            }
        }
        return check;
    ...
    public boolean isUserlogin(HttpServletRequest rq){
    	...
    	if ((b.getClass().isInstance(bib)) || (b.getClass().isInstance(a))) {
            ...
            if (b != null){  
            	...
            	if (ui.getKonto()!=null) {
            	if ((b.isLoginopt()==true) && (ui.getKonto().isUserlogin()==true)) {
            		check = true;
            	}
            } else {
    ...
    public boolean isLegitimateOrder(HttpServletRequest rq, Bestellungen order){
        ...
        if (rq.getSession().getAttribute("userinfo")!=null){
            ...
            if ((!auth.isBenutzer(rq) && order.getId()!=null && ui.getKonto().getId().equals(order.getKonto().getId())) || // Bibliothekare und Admins sehen Bestellungen des Kontos
            	 (auth.isBenutzer(rq) && order.getId()!=null && ui.getBenutzer().getId().equals(order.getBenutzer().getId())) ) { // User sehen nur die eigenen Bestellungen
            	check = true;            	
            }            
        }   
        return check;
    ...
    public boolean isUserSubitoBestellung(HttpServletRequest rq){
    	...
    	if ((b.getClass().isInstance(bib)) || (b.getClass().isInstance(a))) {
            ...
            if (b != null){  
            	 // Sowohl generelle Kontoeinstellungen als auch Berechtigung beim User müssen vorhanden sein...
            	if (ui.getKonto()!=null) {
            	if ((b.isUserbestellung()==true) && (ui.getKonto().isUserbestellung()==true)) {
            		check = true;
            	}
            }            	
            }
    ...
    public boolean isUserGBVBestellung(HttpServletRequest rq){
    	...
    	if ((b.getClass().isInstance(bib)) || (b.getClass().isInstance(a))) {
            ...
            if (b != null){  
            	 // Sowohl generelle Kontoeinstellungen als auch Berechtigung beim User müssen vorhanden sein...
            	if (ui.getKonto()!=null) {
            	if (b.isGbvbestellung()==true && ui.getKonto().isGbvbestellung()==true &&
            		ui.getKonto().getGbvrequesterid()!=null && ui.getKonto().getIsil()!=null) { // für User gibt es nur autom. GBV-Bestellung, deshalb muss requester-id und ISIL vorhanden sein
            		check = true;
            	}
            }            	
            }
    ...
    public boolean isUserAccount(HttpServletRequest rq){
    	...
    	if ((b.getClass().isInstance(bib)) || (b.getClass().isInstance(a))) {
            ...
            if (b != null){  
            	if (ui.getKonto()!=null) {
            	// Sowohl generelle Kontoeinstellungen als auch Berechtigung beim User müssen vorhanden sein...
            	if ((b.isKontostatus()==true) && (ui.getKonto().isKontostatus()==true)) {
            		check = true;
            	}
            } else {
            	// mehrere Kontos, Prüfung kommt nochmals...
    ...
    public boolean isUserAccount(String uid, Connection cn){
        ...
        if (uid.equals("0")) { // kein Kunde ausgewählt = manuelle Bestellung
        	...
        	if (b.isKontostatus()==false) check = false;
        	if (b.isKontostatus()==true) check = true;
        }
        }
