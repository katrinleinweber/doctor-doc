...
public class SpecialCharacters {
    ...
    private String htmlHexToString(String output) {
        ...
        try {
            final Pattern p = Pattern.compile("&#x[a-zA-Z0-9]{2,4};");
            final Matcher m = p.matcher(output);
            final TreeSet<String> set = new TreeSet<String>();
            while (m.find()) {
                final String htmlHex = output.substring(m.start(), m.end());
                set.add(htmlHex);
                //         System.out.println(htmlHex);
            }

            for (String htmlHex : set) {
                final String hex = htmlHex.substring(3, htmlHex.indexOf(";"));
                final int c = Integer.parseInt(hex, 16);
                final char chr = (char) c;
                final String replace = Character.toString(chr);
    ...
    private String htmlDezToString(String output) {
        ...
        try {
            final Pattern p = Pattern.compile("&#[0-9]{2,6};");
            final Matcher m = p.matcher(output);
            final TreeSet<String> set = new TreeSet<String>();
            while (m.find()) {
                final String htmlDez = output.substring(m.start(), m.end());
                set.add(htmlDez);
                //         System.out.println(htmlDez);
            }

            for (String htmlDez : set) {
                final String dez = htmlDez.substring(2, htmlDez.indexOf(";"));
                final int c = Integer.parseInt(dez, 10);
                final char chr = (char) c;
                final String replace = Character.toString(chr);
