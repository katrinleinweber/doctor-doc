    ...
    public OverviewForm checkDateRegion(OverviewForm of, int x) {
		...
		if (this.isExactLength(of.getYfrom(), 4) && this.isExactLength(of.getYto(), 4) && 
			...
			try {
				...
				cal_to.set(Integer.parseInt(of.getYto()), Integer.parseInt(of.getMto()) - 1, Integer.parseInt(of.getDto()), 0, 0, 0);
				cal_from.set(Integer.parseInt(of.getYfrom()), Integer.parseInt(of.getMfrom()) - 1, Integer.parseInt(of.getDfrom()), 0, 0, 0);
				long time = cal_to.getTime().getTime()- cal_from.getTime().getTime();
...
//					System.out.println("Kalender wieder in of setzen, wegen Überläufen (31.2.2008)");
					...
					of.setMfrom(new SimpleDateFormat("MM").format(cal_from.getTime()));
					of.setDfrom(new SimpleDateFormat("dd").format(cal_from.getTime()));
			...
			of.setYfrom(null); // zurückstellen, damit im nächsten Schritt die Default-Werte zum Zug kommen da falsche Längen in den 
		}
    ...
    public OverviewForm checkOrdersSortCriterias(OverviewForm of){
    	if (of.getSort().equals("mail") || of.getSort().equals("artikeltitel") || of.getSort().equals("zeitschrift")
        		...
        		|| of.getSort().equals("konto") || of.getSort().equals("bestellquelle") || of.getSort().equals("mediatype") ){
    ...
    public ArrayList<String> getAlphanumericWordCharacters(String s){
    	...
    	Pattern p = Pattern.compile("([\\p{L}||\\P{Alpha}&&[^\\p{Punct}]&&[^\\p{Space}]])*"); // Regexp: \w 	A word character: [a-zA-Z_0-9]
