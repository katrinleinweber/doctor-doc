...
//          Nachricht erstellen
            ...
            text.setText( "Sehr geehrter Kunde\nSehr geehrte Kundin\n\nAnbei ein von Ihnen per Fax bestellter Artikel im PDF-Format." +"\n\nFuer Reklamationen " + 
            		"wenden Sie sich bitte direkt an den Lieferanten (z.B. Subito) oder an Ihre zustaendige Bibliothek: " + k.getBibliotheksmail() + "\n\nFuer die Einhaltung " + 
            		"der mit den zugestellten Materialien verbundenen Urheber-, Persoenlichkeits- und sonstigen Rechte sind Sie allein " +
            		"verantwortlich. Wir weisen insbesondere darauf hin, dass Sie die von uns zugestellten Dokumentkopien ausschliesslich " +
            		"zum privaten oder sonstigen eigenen Gebrauch verwenden duerfen. Die gelieferten Dokumente duerfen Sie weder entgeltlich noch " +
            ...
            //Fax anhängen
            InputStreamDataSource isds = new InputStreamDataSource(faxId + ".pdf", "application/pdf", conn.getInputStream());
...
//          Mail versenden
    ...
    public void retrieveIncomingFaxList(Konto k){
    	...
    	// FAX [Each fax of fax list is delimited by <FAX></FAX>. The included field are faxId, sender, date and pages count | none]
		...
		try {
			...
			e1.printStackTrace();
			MHelper mh = new MHelper();
	        mh.sendErrorMail("Faxserver Abfrage-URL konnte nicht erstellt werden", "retrieve_incoming_fax_list:  - " + e1.toString() 
	        		+ " KID: " + k.getId() + "Konto: " + k.getBibliotheksname());
...
//    		System.out.println("Faxliste erhalten:\012");
    		...
    		while (contents.contains("<FAX>")) {
    			...
    			contents_copy = contents_copy.substring(contents_copy.indexOf("<FAX>"), contents_copy.indexOf("</FAX>")+6);
    			f.setPopfaxid(contents_copy.substring(contents_copy.indexOf("<FAX_ID>")+8, contents_copy.indexOf("</FAX_ID>")) );
    			if (contents_copy.contains("<FROM>")) f.setFrom(contents_copy.substring(contents_copy.indexOf("<FROM>")+6, contents_copy.indexOf("</FROM>")) );
    			if (contents_copy.contains("<DATE>")) f.setPopfaxdate(contents_copy.substring(contents_copy.indexOf("<DATE>")+6, contents_copy.indexOf("</DATE>")) );
    			if (contents_copy.contains("<PAGES>")) f.setPages(contents_copy.substring(contents_copy.indexOf("<PAGES>")+7, contents_copy.indexOf("</PAGES>")) );
    			...
    			if (f.getFax(f.getPopfaxid(), f.getKid()).getId() == null) { // Prüfen, ob der Eintrag bereits besteht
    				...
    				//Fax per Mail weiterleiten, wenn kein Spam von Popfax (+33170447011 und +4969975392378 sind Popfaxnummern)
    				if (!f.getFrom().equalsIgnoreCase("Popfax.com") && !f.getFrom().contains("+331704") &&
        				...
        				try {
        					synchronized( fh ) {
        					  ...
        					  fh.wait(6000);
	        				  // 6 sekunden warten, da nur alle 5 s auf das Postfach des popfax zugegriffen werden darf
...
//    				System.out.println("\012Fax mit Faxid: " + f.getPopfaxid() + " in DB schon vorhanden!\012");
    			}
    		...
    		MHelper mh = new MHelper();
	        mh.sendErrorMail("!!!Fehler auf Faxserver!!! - Verbindung fehlgeschlagen", "Fehler Faxserver - retrieveIncomingFaxList. API-Antwort:\012" + contents);    		
    	}  
